    ...
    try {
        ...
        Client.cmd.on('ytmp4', async (data) => {
            try {
                ...
                if(data.body == "") return data.reply(`*「 Youtube Downloader 」*\n\nKirim perintah *${data.prefix}ytmp4 [ link ]*\nContoh : ${data.prefix}ytmp4 https://www.youtube.com/watch?v=AsunaBot`)
                data.reply(mess.wait)
                res = await axios.get(`${configs.apiUrl}/api/ytmp4/2?apikey=${configs.zeksKey}&url=${data.body}`)
                if(res.data.status == false) data.reply(res.dakta.message)
                ytm = res.data.result
                teks = `*「 Data Berhasil Diterima! 」*\n\n*Judul* : ${ytm.title}\n*Ukuran* : ${ytm.size}\n*Kualitas* : ${ytm.quality}\n*Ext* : ${ytm.ext}\n\n_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
                if(Number(ytm.size.split(' MB')[0]) >= 50.00) return Client.sendFileFromUrl(data.from, `${ytm.thumb}`, 'thumb.jpg', `*「 Data Berhasil Diterima! 」*\n\n*Title* : ${ytm.title}\n*Ukuran* : ${ytm.size}\n*Kualitas* : ${ytm.quality}\n*Ext* : mp4\n*Link* : ${ytm.link}\n\n_Untuk durasi lebih dari batas disajikan dalam bentuk link_`, data.message)
        ...
        Client.cmd.on('playvid', async (data) => {
            try {
                if(isLimit(data.sender)) return data.reply(mess.limit)
                if(data.body == "") return data.reply(`*「 Youtube Player 」*\n\nKirim perintah *${data.prefix}playvid [ query ]*\nContoh : ${data.prefix}playvid cara bernafas`)
                data.reply(mess.wait)
        ...
        Client.cmd.on('play', async (data) => {
            try {
                if(isLimit(data.sender)) return data.reply(mess.limit)
                if(data.body == "") return data.reply(`*「 Youtube Player 」*\n\nKirim perintah *${data.prefix}play [ query ]*\nContoh : ${data.prefix}play cara bernafas`)
                data.reply(mess.wait)
        ...
        Client.cmd.on('ig', async (data) => {
            if(isLimit(data.sender)) return data.reply(mess.limit)
            if(data.body == "") return data.reply(`*「 Instagram Profile 」*\n\nKirim perintah *${data.prefix}ig [ link ]*\nContoh : ${data.prefix}ig https://instagram.com/AsunaBot`)
            data.reply(mess.wait)
        ...
        Client.cmd.on('igstory', async (data) => {
            try {
                if(isLimit(data.sender)) return data.reply(mess.limit)
                if(data.body == "") return data.reply(`*「 Instagram Story 」*\n\nKirim perintah *${data.prefix}ig [ username ]*\nContoh : ${data.prefix}igstory AsunaBot`)
                data.reply(mess.wait)
        ...
        Client.cmd.on('joox', async (data) => {
            try {
                if(isLimit(data.sender)) return data.reply(mess.limit)
                if(data.body == "") return data.reply(`*「 Joox 」*\n\nKirim perintah *${data.prefix}joox [ lagu ]*\nContoh : ${data.prefix}joox Bad Liar`)
                data.reply(mess.wait)
        ...
        Client.cmd.on('join', async (data) => {
            ...
            Client.acceptInviteLink(data.body).then(() => data.reply('iyh')).catch(() => data.reply('failed'))
        })
